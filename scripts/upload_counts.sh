...
scp root@geoscents.net:/scratch/player_count.csv /scratch/player_count.csv

if [ ! -f "/scratch/player_count.csv" ]; then
    echo "CSV not found: /scratch/player_count.csv"
    exit 1
fi

response=$(curl -s -L -X POST "$APPS_SCRIPT_URL" \
    -H "Content-Type: text/plain" \
    --data-binary @"/scratch/player_count.csv")
